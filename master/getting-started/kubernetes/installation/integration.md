...
- with a "cluster CIDR" that is equal to (or contains) the Calico IP pool.

This ensures that the source address of service-bound packets is preserved.  `iptables` 
mode is the default as of Kubernetes v1.2.0.  Calico's `ipvs` mode support requires at 
least Kubernetes v1.9.3 and it has [some limitations](../installation/#ipvs-mode).

The Kubernetes team are in the process of migrating kube-proxy from command-line argument
